...
if [ "x$GEN_KEYS" == "xtrue" ] ; then
  $TESTJAVA/bin/keytool -genkeypair -alias ca    -keystore ca.jks             -storepass secret -keypass secret -dname cn=ca,dc=redhat,dc=com -keysize 2048 -keyalg RSA -validity 365
  $TESTJAVA/bin/keytool -exportcert -alias ca    -keystore ca.jks             -storepass secret -file ca.cer
  $TESTJAVA/bin/keytool -genkeypair -alias jboss -keystore jboss.keystore.jks -storepass secret -keypass secret -dname "cn=jboss.usersys.redhat.com, ou=GSS,dc=redhat,dc=com" -keysize 2048 -keyalg RSA
  $TESTJAVA/bin/keytool -certreq    -alias jboss -keystore jboss.keystore.jks -storepass secret -file jboss.csr
  $TESTJAVA/bin/keytool -gencert    -alias ca    -keystore ca.jks             -storepass secret -keypass secret -infile jboss.csr -outfile jboss.cer -validity 365
...
$TESTJAVA/bin/java $OPTS  HTTPSClient 2>&1 | tee clientLog1
$TESTJAVA/bin/java $OPTS $ANCHOR  HTTPSClient 2>&1 | tee clientLog2
