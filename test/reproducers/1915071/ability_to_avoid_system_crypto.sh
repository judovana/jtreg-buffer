...
if [ "x$OTOOL_JDK_VERSION" = "x8" ] ; then
  $TESTJAVA/bin/keytool -J-Djava.security.disableSystemPropertiesFile $SHARED_SETUP -storetype PKCS12 -dname "$DNAME"  $KEYS -keystore my3.keystore
else
  $TESTJAVA/bin/keytool -J-Djava.security.disableSystemPropertiesFile $SHARED_SETUP -storetype jks    -dname "$DNAME"  $KEYS -keystore my.keystore 
fi
